  ...
  async (data: { productId: number; quantity: number }, { getState }) => {
    ...
    if (found) {
      const response = await api.post(
        '/v1/cart/items/',
        { id: data.productId, quantity: found.quantity + data.quantity },
        { headers: { Nonce: state.global.nonce } }
      );
      return response.data;
    } else {
      const response = await api.post(
        '/v1/cart/items',
        { id: data.productId, quantity: data.quantity },
        { headers: { Nonce: state.global.nonce } }
      );
      return response.data;
    }
...
export const decrementCartItem = createAsyncThunk(
  ...
  async (data: { productId: number; quantity: number }, { getState }) => {
    ...
    if (found && found.quantity > 1) {
      const response = await api.post(
        `/v1/cart/items/`,
        { id: data.productId, quantity: found.quantity - 1 },
        { headers: { Nonce: state.global.nonce } }
      );
